...
**Evidence.** Every rule below traces to one of: a file in an addon under `reference/`, a numbered
transferable rule in `findings/reference/*.md`, a section of `ui-vocabulary.md`, or the current
behaviour of `statamic/cms` 6.x (`reference/statamic__cms/`). Citations are inline.
...
- **`"type": "statamic-addon"`.** Statamic discovers addons by package type. `bard-texstyle` omits it
  (`findings/reference/jacksleight-statamic-bard-texstyle.md` §10.8) — harmless until tooling keys off it.
...
(`findings/reference/statamic-collaboration.md` R21, `statamic-seo-pro.md` R27). `bard-texstyle` and
...
(`findings/reference/aerni-advanced-seo.md` R9, `statamic-rad-pack-mailchimp.md` R1). An explicit
...
(`findings/reference/aryehraber-statamic-logbook.md` R18).
...
(`findings/reference/statamic-rad-pack-runway.md` §10.5). If the extension point does not exist, open
...
`bard-texstyle` all converge on this shape (`findings/reference/statamic-seo-pro.md` R5,
`aerni-advanced-seo.md` R8, `jacksleight-statamic-bard-texstyle.md` R23). Keep `bootAddon()` itself to
...
- **`register()`** — container bindings, `registerSerializableClasses()`, and nothing that reads
  ...
  late (`findings/reference/statamic-eloquent-driver.md` §10.2). If you must merge manually, merge in
  `register()`.
- **`bootAddon()`** — everything that touches a Statamic facade: `Nav::`, `Permission::`, `Form::`,
  ...
  core's `Statamic::booted()` chain (`findings/reference/statamic-rad-pack-mailchimp.md` R3,
  `aryehraber-statamic-logbook.md` R1).
...
- **No `Route::get|post|patch|delete` in the provider.** Route *definitions* go in `routes/cp.php`;
  only `Route::bind()` belongs in the provider (`findings/reference/aerni-advanced-seo.md` R11).
...
- **The merge key must be the exact prefix every `config()` call reads.** `eloquent-driver` merges into
  ...
  (`findings/reference/statamic-eloquent-driver.md` §10.1).
...
- **Every config read has an inert default:** `config('thing.driver', 'file')`. An unpublished,
  ...
  (`findings/reference/statamic-eloquent-driver.md` R3).
- **Secrets come from `env()` in the config file, never from a settings blueprint field**
  (`findings/reference/statamic-rad-pack-mailchimp.md` R8).
- **Never hand a whole config array to the frontend.** `Statamic::provideToScript()` takes named keys
  you have chosen, not `config('thing')` (`findings/reference/jacksleight-statamic-bard-texstyle.md` R9).
...
(`findings/reference/statamic-eloquent-driver.md` R1–R17):
...
- **Expose every swappable behaviour as an interface bound in the container**, so a test can replace it
  with one `$this->mock()` line (`findings/reference/spatie-statamic-responsive-images.md` R12).
...
- **Facades.** A facade over a `singleton()` binding is the right way to expose a programmatic API that
  ...
  (`findings/reference/statamic-ssg.md` R18). If you hand-maintain `@method` docblocks on it, pin them
  with a test — `mailchimp`'s facade docblock has drifted from its driver by three methods
  (`findings/reference/statamic-rad-pack-mailchimp.md` §10.13).
...
- **Caching.** Every negative lookup you cache must be evicted immediately, and every write must
  invalidate its keys (`findings/reference/statamic-eloquent-driver.md` R15). Use `Blink` for
  request-scoped memoisation; do not use `Cache::forever` as a coordination log between queue workers
  (`findings/reference/statamic-importer.md` §10.10 — a lost-update race by construction).
...
- **Convention-based wiring has no compile-time safety net.** Add a test per listener that dispatches
  the real event and asserts the effect (`findings/reference/statamic-rad-pack-mailchimp.md` R18).
...
(`findings/reference/aerni-advanced-seo.md` R16). Do not `array_replace_recursive` a foreign blueprint
...
- **Register serialisable domain objects.** Any Statamic object a job holds must be listed in
  ...
  (`findings/reference/statamic-seo-pro.md` R22).
- **Give every job a public accessor for its payload** (`getParams()`), so tests can write
  ...
  (`findings/reference/spatie-statamic-responsive-images.md` R13).
...
- **Restore global state in a `finally`.** Loops that mutate `Site::setCurrent()`, `app()->setLocale()`
  ...
  (`findings/reference/statamic-ssg.md` R20).
...
- **Always a named `Permission::group()`.** Ungrouped permissions land in a miscellaneous bucket in the
  role editor (`findings/reference/statamic-rad-pack-runway.md` R10).
- **Hierarchy, not a flat list:** `view` → `edit` → `create`/`delete`
  (`findings/reference/duncanmcclean-simple-commerce.md` R9).
- **`label()` and `description()` from `lang/` keys**, never bare English
  (`findings/reference/aerni-advanced-seo.md` R15).
- **Register only permissions that apply to the configured feature set**, and gate routes, nav items,
  ...
  (`findings/reference/duncanmcclean-simple-commerce.md` R11).
- **A settings blueprint gives you `edit {package} settings` for free**
  ...
  (`findings/reference/aryehraber-statamic-logbook.md` R2, R4). Do not re-register either.
...
(`findings/reference/duncanmcclean-simple-commerce.md` §10.A1).
...
`findings/reference/statamic-rad-pack-runway.md` R11).
...
`edit {package} settings` permission (`findings/reference/statamic-rad-pack-mailchimp.md` R4, R5).
...
(`findings/reference/statamic-rad-pack-mailchimp.md` §10.1). It is exactly the page the settings
blueprint replaced.
...
(`findings/reference/jacksleight-statamic-bard-texstyle.md` R16).
...
- No orphan views: every file under `resources/views/` is referenced by a controller or `view()` call
  (`findings/reference/statamic-rad-pack-mailchimp.md` R16) — *(manual)*
...
- **Extend core controllers**: `Statamic\Http\Controllers\CP\CpController`, `ActionController`,
  `PreviewController` (`findings/reference/statamic-rad-pack-runway.md` R13).
- **Validate through the blueprint, not the request.**
  ...
  (`findings/reference/aerni-advanced-seo.md` R7, `statamic-importer.md` R13,
  `statamic-rad-pack-runway.md` R11). Non-blueprint endpoints — utilities, one-off actions — use a
  `FormRequest`; **never nothing** (`findings/reference/aryehraber-statamic-logbook.md` R15).
- **Row-dependent validation belongs in the fieldtype's `extraRules()`**, not the controller
  (`findings/reference/statamic-importer.md` R14).
- **Never build CP URLs by hand.** `cp_route()` / `cp_url()` in PHP; pass URLs to Vue as props. Vue must
  not concatenate `/cp/…` (`findings/reference/aryehraber-statamic-logbook.md` R3,
  `statamic-rad-pack-runway.md` R12).
- **Bind domain objects with `Route::bind()`** in the provider, guarded with `Statamic::isCpRoute()`,
  ...
  (`findings/reference/statamic-seo-pro.md` R23, `statamic-rad-pack-runway.md` R15).
- **Resolve permissions server-side into boolean props.** No `can(` in Vue
  (`findings/reference/statamic-seo-pro.md` R8).
- **Index and its JSON come from the same route**, branched on `$request->wantsJson()`, so the
  `<Listing>` `:url` and the page are never out of sync (`findings/reference/statamic-seo-pro.md` R9).
- **Format listing cells server-side** through the blueprint's index fieldtypes:
  ...
  (`findings/reference/duncanmcclean-simple-commerce.md` R6, `statamic-seo-pro.md` R11).
...
never binding — `findings/reference/aryehraber-statamic-logbook.md` §10.1–10.2); and `logbook`'s own
...
- **Test both.** A single-site test suite proves nothing about the multi-site path. `advanced-seo`'s
  ...
  `$siteFixture`, `findings/reference/statamic-ssg.md` R21) are the two workable patterns.
- **Do not derive context from the browser URL.** `mailchimp`'s `FormFieldsFieldtype.vue:48` reads
  ...
  (`findings/reference/statamic-rad-pack-mailchimp.md` §10.6). Pass it through `preload()`.
...
(`findings/reference/statamic-ssg.md` R11–R17):
...
- Multi-part import/export commands get `--force` plus one `--only-<part>` per part, with matching
  ...
  (`findings/reference/statamic-eloquent-driver.md` §10.12).
...
- **Never `catch` into silence.** `mailchimp` returns `[]` from `MailchimpField::callApi()` when the API
  ...
  empty dropdown with no explanation (`findings/reference/statamic-rad-pack-mailchimp.md` §10.8). Show
...
- **`match` on untrusted input needs a `default`.** `importer` matches on `Storage::mimeType()` in four
  ...
  message (`findings/reference/statamic-importer.md` §10.4).
- **Decryption and decoding of user input is wrapped**, returning 404/422 rather than 500
  (`findings/reference/aryehraber-statamic-logbook.md` R14).
- **404, consistently.** `seo-pro`'s `show()` uses `throw_unless(…, NotFoundHttpException::class)` while
  ...
  (`findings/reference/statamic-seo-pro.md` §10.11). Pick the guard and apply it to every action,
...
- **Index and preview rendering must be total.** Catch domain exceptions in `preProcessIndex()` and
  ...
  one row is broken (`findings/reference/spatie-statamic-responsive-images.md` R23).
- **Public APIs do not silently no-op.** `eloquent-driver`'s `with()` returns `$this` and discards the
  eager-load request (`findings/reference/statamic-eloquent-driver.md` §10.8). Throw, or implement.
- **Ship no debug statements.** `dd()`, `dump()`, `ray()`, `console.log` reach a customer's production
  ...
  (`findings/reference/statamic-collaboration.md` §10.12).
- **Sandbox any iframe rendering user-authored content** (`ui.unsandboxed-iframe`), and **confirm every
  ...
  (`ui-vocabulary.md` §6; `findings/reference/aryehraber-statamic-logbook.md` R20).
...
Rules from `eloquent-driver` (`findings/reference/statamic-eloquent-driver.md` R23–R25) and
`simple-commerce` (R25):
...
- **Every class in `src/UpdateScripts/` runs, and every class listed in `$updateScripts` exists.**
  ...
  (`findings/reference/statamic-eloquent-driver.md` §10.3).
...
- **Deprecations need an owner and a deadline.** `eloquent-driver` carries five parallel legacy-config
  ...
  (`findings/reference/statamic-eloquent-driver.md` §10.6).
...
(`findings/reference/spatie-statamic-responsive-images.md` R1–R2, and R17/R21/R22/R23/R25 across five
more references).
...
(`findings/reference/jacksleight-statamic-bard-texstyle.md` §10.2). Do not keep a `phpunit.xml`
claiming a PHPUnit 9.3 schema under a Pest 4 project
(`findings/reference/aerni-advanced-seo.md` §10.11).

### `phpunit.xml`

Pin the environment in the file so no `.env.testing` is needed, and turn the strict flags on
(`findings/reference/spatie-statamic-responsive-images.md` R6–R7, `statamic-ssg.md` R24):
...
exist in a package (`findings/reference/statamic-rad-pack-mailchimp.md` §10.10). Watch the
`suffix="Test.php"` trap: `runway` has a controller test file without the suffix, so restore-revision
is silently untested (`findings/reference/statamic-rad-pack-runway.md` §10.10).
...
1. **Every CP endpoint, including the unauthorized case.** For each route in `routes/cp.php`: a 200 for
   ...
   (`findings/reference/statamic-importer.md` R25, `statamic-seo-pro.md` R25,
   `duncanmcclean-simple-commerce.md` R23). This is the gap in *every* reference addon — `advanced-seo`
...
7. **Commands**: `$this->artisan(Cmd::class)->expectsOutput(…)->assertExitCode(0)`, including the
   precondition-not-met path (`findings/reference/spatie-statamic-responsive-images.md` R14).
...
- Copy binary fixtures into a temp dir in `setUp()`, delete and recreate that dir in **both** `setUp()`
  ...
  (`findings/reference/spatie-statamic-responsive-images.md` R10–R11).
...
- Fake every outbound HTTP call. Never inject a live API key into CI — `mailchimp` does, and the suite
  ...
  (`findings/reference/statamic-rad-pack-mailchimp.md` R19, §10.12).
...
(`findings/reference/statamic-collaboration.md` §10.2).
...
- **`prefer-lowest` is not optional.** Without it the declared floor (`statamic/cms: ^6.25`) is never
  ...
  (`findings/reference/spatie-statamic-responsive-images.md` §10.2).
- **The PHP floor in the matrix must equal the `require.php` lower bound**
  (`findings/reference/statamic-rad-pack-mailchimp.md` R22).
...
- **Every npm script a workflow calls must exist in `package.json`.**
  ...
  (`findings/reference/spatie-statamic-responsive-images.md` R19, §10.3).
- **If `dist/` is committed, CI rebuilds it and fails on a diff:**
  ...
  users load a stale bundle (`findings/reference/eminos-statamic-tabs.md` R21,
  `jacksleight-statamic-bard-texstyle.md` §10.1).
...
- Dependabot on `github-actions` **and** `composer` **and** `npm` — `collaboration` watches only
  ...
  (`findings/reference/statamic-collaboration.md` §10.13).
...
(`findings/reference/spatie-statamic-responsive-images.md` §10.5). The studio adopts it as a default
...
- **Types on the public surface.** Every method in `src/Contracts/`, `src/Facades/` and every
  ...
  (`findings/reference/eminos-statamic-tabs.md` R24).
...
- **`class_exists()` guards on every optional integration.** Any reference to a class from a
  ...
  a production site (`findings/reference/duncanmcclean-simple-commerce.md` R12,
  `statamic-ssg.md` R6). Optional integrations belong in `require-dev` + `suggest`, never `require`
  (`findings/reference/aerni-advanced-seo.md` R24).
- **Depend on what you import.** `ssg` imports `wilderborn/partyline` in five files and does not
  ...
  (`findings/reference/statamic-ssg.md` §10.3).
...
(`findings/reference/statamic-ssg.md` R29). Matching core's formatting means a diff between your addon
...
- Scope Pint to the **whole package**, not just `src/`. `spatie/responsive-images` scopes its
  ...
  (`findings/reference/spatie-statamic-responsive-images.md` §10.6).
...
(`findings/reference/aerni-advanced-seo.md` §10.10,
`spatie-statamic-responsive-images.md` §10.4). `ssg` runs Pint in `testMode` on PRs with no commit step
(`findings/reference/statamic-ssg.md` R28). **The studio picks check-and-fail.** Auto-fix rewrites
...
- **Pin the Pint version identically** in `composer.json` `require-dev` and in the lint workflow
  (`findings/reference/statamic-seo-pro.md` R26).
...
- **Ship `lang/en/*.php`.** Namespaced keys without a `lang/` directory render the raw key in the CP.
  ...
  buys a host app nothing (`findings/reference/spatie-statamic-responsive-images.md` §10.10);
...
- **`lang/`, not `resources/lang/`.** Core checks `lang/` first
  ...
  (`findings/reference/statamic-seo-pro.md` §10.3).
- **Namespaced keys for addon copy** (`__('thing::messages.saved')`), **bare core keys for verbs core
  ...
  value duplicates a core value is a bug (`findings/reference/aerni-advanced-seo.md` R20).
- **Wrap everything, including the strings that are easy to forget**: empty states, flash messages,
  ...
  literal (`findings/reference/statamic-collaboration.md` §10.5); `seo-pro` `__()`-wraps everything
  except one error message rendered directly in a modal (`findings/reference/statamic-seo-pro.md` §10.12).
- **Validation closures translate:** `$fail('thing::validation.key')->translate()`
  (`findings/reference/statamic-importer.md` R20).
- **Utility `->title()`/`->description()` must be `__()`-wrapped**, because those exact strings become
  the permission label and description (`findings/reference/aryehraber-statamic-logbook.md` R5).
