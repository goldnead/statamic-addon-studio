...
return [
    ...
    */
    ...
    /*
    | DEMO-ENTSCHEIDUNG (25.08.2026): Der Treiber bleibt `flat`.
    |
    | Eine Liste und eine Kampagne sind Definitionen, keine Laufzeitdaten. Sie
    | gehoeren in dieselbe Schublade wie der Produktkatalog, der in diesem
    | Playground bewusst in `config/statamic-payments.php` steht und nicht in
    | eine Tabelle. `flat` ist ausserdem die Vorgabe des Addons: ein Schauraum,
    | der die Vorgabe umgeht, um vorzeigbar zu sein, zeigt nicht das Addon.
    |
    | Zweitens ist es die Fassung mit den schaerferen Kanten, und genau die soll
    | ein Demo fahren: Markenverzeichnis im Pfad, `readSegments()` faellt zu,
    | wenn keine Marke aufgeloest ist, und `guardHandleIsFree()` verweigert ein
    | Handle, das eine andere Marke schon haelt. Unter `eloquent` wuerde nichts
    | davon laufen.
    |
    | Die vier Zeilen in `marketing_lists`, die der alte Seeder per Eloquent
    | geschrieben hat, waren unter diesem Treiber unsichtbar. SeedsCampaign
    | raeumt sie weg und schreibt die Listen ueber das Repository.
    */
    'storage' => [
    ...
    */
    ...
    'subscriptions' => [
        ...
        'confirmation_throttle' => [
            ...
            // Bleibt null = Anwendungsvorgabe. Die steht in diesem Playground
            // seit dem 25.08.2026 auf `database` (vorher `file`): FileStore ist
            // nicht in `ConfirmationThrottle::ATOMIC_STORES`, und damit hat das
            // Addon JEDE Doppel-Opt-in-Mail zurueckgehalten.
